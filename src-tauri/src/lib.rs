...
use crate::contracts::{
  AppBootstrap, JobLookup, JoinJobStatus, JoinPreparationRequest, JoinPreparationResult, LaunchSettings,
  ListServersRequest, PaginatedServersResponse, RequiredMod, ServerDetails, ServerLookup, ServerRecord,
};
...
async fn prepare_join_impl(state: AppState, request: JoinPreparationRequest) -> Result<JoinPreparationResult, AppError> {
  ...
  if matches!(launch_mode, crate::contracts::LaunchMode::DirectProton) && request.settings.preferred_proton_path.is_none() {
    if let Some(path) = proton_path {
      warnings.push(format!("Auto-detected Proton runtime at {path} for direct join launch."));
    }
  }
  Ok(JoinPreparationResult {
    ...
    warnings,
    resolved_mods,
    ready_to_launch: details.required_mods.is_empty() || details.required_mods.iter().all(|item| item.workshop_id.chars().all(|character| character.is_ascii_digit())),
  })
}

async fn run_launch_job(state: AppState, request: JoinPreparationRequest, job_id: String) -> Result<(), AppError> {
  let bootstrap_state = bootstrap(&state.db_path)?;
  let dayz = bootstrap_state
    .dayz_install
    .ok_or_else(|| AppError::new("DayZ install could not be detected"))?;
  ...
  if !missing_mods.is_empty() {
    ...
    if SteamworksAdapter::available() {
      ...
      status.message = String::from("Subscribing to missing workshop mods through Steamworks");
      upsert_job(&state, status.clone())?;
      ...
      .await
      {
        Ok(Ok(subscription_result)) => {
          ...
          if subscription_result.subscribed_ids.is_empty() {
            status.warnings.push(String::from("Steamworks did not accept any missing mods for subscription."));
          }
          ...
          status.message = if guided_fallback_ids.is_empty() {
            ...
            String::from("Steam subscribed some mods. Opening guided fallback for the rest.")
          };
          upsert_job(&state, status.clone())?;
        }
        Ok(Err(error)) => {
          status.warnings.push(format!("Steamworks subscription failed: {error}"));
          status.warnings.push(String::from("Falling back to guided Steam flow."));
          upsert_job(&state, status.clone())?;
        }
        Err(error) => {
          status
            .warnings
            .push(format!("Steamworks task failed before subscription completed: {error}"));
          status.warnings.push(String::from("Falling back to guided Steam flow."));
          upsert_job(&state, status.clone())?;
        }
      }
    ...
    if !guided_fallback_ids.is_empty() {
      ...
      status.message = String::from("Opened remaining workshop items in Steam. Waiting for local installs.");
      upsert_job(&state, status.clone())?;
    } else {
      status.phase = String::from("steam-download");
      status.progress = 0.64;
      status.message = String::from("Waiting for Steam to finish downloading subscribed workshop mods.");
      upsert_job(&state, status.clone())?;
    }
