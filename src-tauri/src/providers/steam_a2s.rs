...
pub async fn fetch_snapshot(ip: &str, port: u16, wait: Duration) -> Result<A2sSnapshot, AppError> {
  ...
  let socket = UdpSocket::bind("0.0.0.0:0").await.map_err(|error| AppError::new(error.to_string()))?;
...
fn parse_info_response(packet: &[u8]) -> Result<A2sInfo, AppError> {
  ...
  cursor += 2;
  let players = packet.get(cursor).copied().ok_or_else(|| AppError::new("Missing players field"))?;
  cursor += 1;
  let max_players = packet.get(cursor).copied().ok_or_else(|| AppError::new("Missing max players field"))?;
  cursor += 1;
  cursor += 5;
...
fn read_c_string(packet: &[u8], cursor: &mut usize) -> Result<String, AppError> {
  ...
  while let Some(byte) = packet.get(end) {
    if *byte == 0 {
      let value = String::from_utf8(packet[start..end].to_vec()).map_err(|error| AppError::new(error.to_string()))?;
      *cursor = end + 1;
      return Ok(value);
    }
    end += 1;
