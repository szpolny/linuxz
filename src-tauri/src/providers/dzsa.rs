...
pub async fn fetch_details_for_server(
    ...
    for port in candidates {
        ...
        join_set.spawn(async move {
            ...
            if payload.status == 0 {
                return payload.result.ok_or_else(|| {
                    AppError::new("DZSA returned success without a result payload")
                });
            }

            Err(AppError::new(payload.error.unwrap_or_else(|| {
                String::from("DZSA returned an error response")
            })))
        });
    }
    ...
    Err(AppError::new(last_error.unwrap_or_else(|| {
        String::from("DZSA did not return server metadata for any attempted port")
    })))
}
...
mod tests {
    ...
    fn parses_live_like_dzsa_payload_with_mods() {
      ...
      "result": {
        ...
        let response =
            serde_json::from_str::<DzsaResponse>(payload).expect("dzsa payload should deserialize");
