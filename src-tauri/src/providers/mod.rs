...
pub async fn get_server_details(
  ...
  if let Ok(snapshot) = steam_a2s::fetch_snapshot(&server.ip, server.query_port, Duration::from_secs(2)).await {
    ...
    details.server.connect_port = Some(server.connect_port.unwrap_or(server.query_port.saturating_sub(3)));
    details.provider_provenance.push(String::from("a2s-info"));
  } else {
    details.warnings.push(String::from("A2S live query failed; using cached browser values."));
  }

  if enable_dzsa {
    match dzsa::fetch_details_for_server(&server.ip, server.query_port, server.connect_port).await {
      Ok(extra) => {
        ...
        details.rules.insert(String::from("battleye"), extra.battleye.to_string());
        details.rules.insert(String::from("time"), extra.time.clone());
      ...
      Err(_) => {
        details.warnings.push(String::from("DZSA enrichment failed for this server."));
      }
    }
  }
...
async fn enrich_live_browser_data(servers: Vec<ServerRecord>) -> Vec<ServerRecord> {
  ...
  for (index, server) in servers.into_iter().enumerate() {
    join_set.spawn(async move {
      let snapshot = steam_a2s::fetch_snapshot(&server.ip, server.query_port, Duration::from_millis(900)).await.ok();
