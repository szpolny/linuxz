...
pub fn reconcile_mods(dayz: &DayzInstall, required: &[RequiredMod]) -> Result<Vec<RequiredMod>, AppError> {
  ...
  Ok(
    required
      ...
      .map(|item| {
        ...
        let install_path = mod_install_path(Path::new(&dayz.workshop_manifest_path), &item.workshop_id);
