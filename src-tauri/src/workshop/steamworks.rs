...
mod implementation {
  ...
  impl SteamworksAdapter {
    ...
    pub fn subscribe_items(workshop_ids: &[String]) -> Result<SteamworksSubscriptionResult, AppError> {
      ...
      let ugc = client.ugc();
      let callback_results = Arc::new(Mutex::new(BTreeMap::<u64, Option<Result<(), String>>>::new()));
      ...
      for workshop_id in workshop_ids {
        ...
        if state.intersects(ItemState::SUBSCRIBED | ItemState::INSTALLED | ItemState::DOWNLOADING | ItemState::DOWNLOAD_PENDING) {
      ...
      while Instant::now() < deadline && !pending_ids.is_empty() {
        ...
        for published_file_id in &pending_ids {
          ...
          if let Some(Some(Err(error))) = callback_snapshot.get(published_file_id) {
            ...
            result
              .warnings
              .push(format!("Steamworks subscribe failed for {published_file_id}: {error}"));
            resolved_ids.push(*published_file_id);
          }
        }
      ...
      for published_file_id in pending_ids {
        ...
        match callback_results.get(&published_file_id) {
          ...
          Some(Some(Err(error))) => {
            ...
            result
              .warnings
              .push(format!("Steamworks subscribe failed for {workshop_id}: {error}"));
          }
          _ => {
            result.failed_ids.push(workshop_id.clone());
            result
              .warnings
              .push(format!("Steamworks did not mark {workshop_id} as subscribed before timeout."));
          }
        }
      }
...
mod implementation {
  ...
  impl SteamworksAdapter {
    ...
    pub fn subscribe_items(_workshop_ids: &[String]) -> Result<SteamworksSubscriptionResult, AppError> {
      Err(AppError::new("Steamworks support is not compiled into this build"))
    }
  }
}
