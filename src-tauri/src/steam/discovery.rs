...
pub fn bootstrap(db_path: &Path) -> Result<AppBootstrap, AppError> {
  ...
  let warnings = if installs.is_empty() {
    ...
    vec![String::from("Steam was found, but DayZ app 221100 was not detected in any scanned library.")]
  } else {
    Vec::new()
  };
...
pub fn detect_steam_installs() -> Result<Vec<SteamInstallResolved>, AppError> {
  ...
  for (kind, root) in candidate_roots() {
    ...
    let has_dayz = libraries.iter().any(|library| library.join(format!("appmanifest_{DAYZ_APP_ID}.acf")).exists());
...
fn candidate_roots() -> Vec<(&'static str, PathBuf)> {
  ...
  if let Some(home) = dirs::home_dir() {
    ...
    roots.push(("flatpak", home.join(".var/app/com.valvesoftware.Steam/.local/share/Steam")));
  }
  roots
}
...
fn resolve_dayz_install(root: &Path, libraries: &[PathBuf]) -> Result<Option<DayzInstall>, AppError> {
  for library in libraries {
    ...
    let install_dir = extract_value(&manifest, "installdir").unwrap_or_else(|| String::from("DayZ"));
...
fn select_install<'a>(
  ...
  if let Some(preferred_id) = preferred_id {
    if let Some(match_install) = installs.iter().find(|install| install.install.id == preferred_id) {
...
mod tests {
  ...
  fn extracts_manifest_value() {
    ...
    assert_eq!(extract_value(manifest, "installdir").as_deref(), Some("DayZ"));
  }

  #[test]
  fn parses_quoted_fields() {
    let fields = quoted_fields("\"path\"\t\t\"/games/SteamLibrary\"");
    assert_eq!(fields, vec!["path".to_string(), "/games/SteamLibrary".to_string()]);
  }
}
